...
from pdf_generator import generate_pdf, download_image, create_pdf_with_scenes, pdf_to_images, create_gelato_pdf
...
def create_script():
    try:
        ...
        # Find the matching credits needed based on selected_tokens
        credits_needed = None
        ...
        story_response = generate_story_text(data)
        ...
        # 6. Ladda upp omslagsbild till S3 (anropet till skapa_omslagsbild() är borttaget)

        # Hämta avatar-nyckeln från data eller sätt en standard
        avatar_url = data.get('avatar', None)
        avatar_key = os.path.splitext(avatar_url.strip('/').split('/')[-1])[0] if avatar_url else "male1"

        # Kontrollera om omslagsbilds-URL redan finns från generate_story_text()
        omslag_image_url = story_response.get('omslag_image_url', None)

        omslag_image_s3_url = None

        if omslag_image_url:
            title_sanitized = sanitize_filename(title)  # Se till att namnet är identiskt i hela flödet
            cover_image_path = os.path.join(TEMP_DIR, f"{title_sanitized}_cover.png")

            # ✅ Hämta book_folder från story_response
            book_folder = story_response.get('book_folder')

            # ✅ Använd den befintliga book_folder istället för att skapa en ny
            omslag_local_path = download_image(omslag_image_url, f"{title}_cover.png", book_folder)

            if omslag_local_path:
                s3_cover_key = f"{user_id}/covers/{title_sanitized}.png"
                omslag_image_s3_url = upload_binary(omslag_local_path, s3_cover_key, file_type="image/png")

                logger.info(f"✅ Omslagsbild sparad i temp_images och uppladdad till S3: {omslag_image_s3_url}")

        # 7. Generera ljudbok (om relevant)
        audio_s3_url = None
        ...
        if book_type in ['pdf', 'both']:
            ...
            # Konvertera story_response till dictionary om det behövs
            if isinstance(story_response, str):
                try:
                    story_response = json.loads(story_response)
                except json.JSONDecodeError:
                    logger.error("🚨 Fel: story_response är en sträng men kunde inte konverteras till dictionary!")
                    return jsonify({"error": "Misslyckades att skapa PDF: story_response är ogiltig."}), 500
            ...
            try:
                ...
                # Kontrollera om backside image finns i story_response
                backside_image_url = story_response.get('backside_image_url')
                if backside_image_url:
                    # Ladda ner backside image till book_folder
                    backside_local_path = download_image(
                        backside_image_url,
                        f"{title_sanitized}_backside.png",  # Viktigt: Använd samma namnformat som i save_backside_image_locally
                        book_folder
                    )
                    logger.info(f"✅ Backside image downloaded to: {backside_local_path}")
                else:
                    logger.warning("⚠️ No backside image URL found in story_response")
                    backside_local_path = None

                # Skapa PDF med backside image
                pdf_path = save_cover_locally_and_generate_pdf(story_response, f"./pdf_books/{title}.pdf")

                # Ladda upp PDF till S3
                s3_pdf_key = f"{user_id}/pdf/{title}.pdf"
                pdf_url = upload_binary(pdf_path, s3_pdf_key, file_type='application/pdf')
                logger.info(f"✅ PDF genererad och uppladdad till S3: {pdf_url}")

            except Exception as e:
                logger.error(f"❌ Misslyckades att skapa PDF: {e}")
                logger.exception(e)  # Ger mer detaljerad felinformation
        ...
        new_story = Story(
            ...
            #insida_image_urls=json.dumps(scene_images)  # ✅ Sparar scenbilds-URL:er i databasen
        )
        ...
        # 11. Rensa temporära bilder **efter att allt är klart**
       # clean_temp_images()
